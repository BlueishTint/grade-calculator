<script lang="ts">
	...
	let assignments = $state([
		...
		{ id: 3, name: 'Midterm 2', weight: '12.5*2', grade: '75/80*100', effort: 1 },
		{ id: 4, name: 'Final Exam', weight: '30', grade: '', effort: 1.5 }
	]);
	...
	let processedData = $derived.by(() => {
		const items = assignments.map((a) => {
			// New: Parse the weight expression
			const parsedWeight = parseWeight(a.weight);
			const parsed = parseGrade(a.grade);
			return { ...a, parsedWeight: parsedWeight, ...parsed };
		});
